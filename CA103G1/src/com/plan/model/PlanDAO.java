...
public class PlanDAO implements PlanDAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO plan(plan_id,mem_id,plan_name,plan_vo,plan_cover,plan_start_date,plan_end_date,sptype_id,plan_view,plan_privacy,plan_create_time,plan_status) VALUES ('PLAN'||LPAD(to_char(plan_seq.NEXTVAL),6,'0'),?,?,?,?,?,?,?,?,?,sysdate,'PLANST0')";
	...
	/********************* 1015 給訪客/好友的個人頁面 *****************/
		private static final String GET_PLAN_FOR_VISITOR_BY_MEM = "SELECT plan_id,mem_id,plan_name,plan_vo,plan_cover,to_char(plan_start_date ,'yyyy-mm-dd hh24:mi:ss')plan_start_date,to_char(plan_end_date,'yyyy-mm-dd hh24:mi:ss')plan_end_date,sptype_id,plan_view,plan_privacy,to_char(plan_create_time,'yyyy-mm-dd')plan_create_time,plan_status  FROM plan  where plan_privacy = 'PLANPR0' AND mem_id=? order by plan_id";
		private static final String GET_PLAN_FOR_FRIEND_BY_MEM = "SELECT plan_id,mem_id,plan_name,plan_vo,plan_cover,to_char(plan_start_date ,'yyyy-mm-dd hh24:mi:ss')plan_start_date,to_char(plan_end_date,'yyyy-mm-dd hh24:mi:ss')plan_end_date,sptype_id,plan_view,plan_privacy,to_char(plan_create_time,'yyyy-mm-dd')plan_create_time,plan_status  FROM plan  where (plan_privacy = 'PLANPR0' OR plan_privacy = 'PLANPR2') AND mem_id=? order by plan_id";
	/*************************************************/
...
//			throw new RuntimeException("A database error occurred. " + se.getMessage());
	...
	public List<PlanVO> getPlanForVisitorByMem(String mem_id) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_PLAN_FOR_VISITOR_BY_MEM);
			pstmt.setString(1,mem_id);
			rs = pstmt.executeQuery();
	...
	public List<PlanVO> getPlanForFriendByMem(String mem_id) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(GET_PLAN_FOR_FRIEND_BY_MEM);
			pstmt.setString(1,mem_id);
			rs = pstmt.executeQuery();
